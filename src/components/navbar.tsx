...
function NavLinks({
  ...
  const messagesHref = user ? "/messages" : "/login?next=/messages";
  ...
  return (
    <>
      ...
      {user && !mobile && <NotificationBell unread={unread} />}

      <Link href={messagesHref} className={linkClass} aria-label="Messages">
        <svg
